...
class TiendaController {
    ...
    private val productos = mutableListOf(
        Producto(1, "Laptop", "HP", "Laptop 15 pulgadas, 8GB RAM, SSD 512GB", 850.00, 5),
        Producto(2, "Mouse inalámbrico", "Logitech", "Mouse óptico inalámbrico USB", 15.99, 20),
        Producto(3, "Teclado mecánico", "Redragon", "Teclado RGB con switches azules", 45.50, 10),
        Producto(4, "Monitor", "Samsung", "Monitor LED 24 pulgadas Full HD", 180.00, 7),
        Producto(5, "Audífonos Bluetooth", "Sony", "Audífonos inalámbricos con cancelación de ruido", 60.00, 12),
        Producto(6, "Memoria USB 64GB", "Kingston", "USB 3.0 alta velocidad", 12.75, 25),
        Producto(7, "Disco SSD 1TB", "Crucial", "SSD interno alta velocidad", 120.00, 8),
        Producto(8, "Webcam HD", "Logitech", "Cámara 1080p para videollamadas", 35.00, 9)
    )
    ...
    private fun mostrarProductos() {
        ...
        val headers = listOf("Código", "Nombre", "Marca", "Descripción", "Precio Uni", "Estado", "Stock")

        val todas = listOf(headers) + filas
        val anchos = (0 until headers.size).map { col ->
            todas.maxOf { it[col].length }.coerceAtMost(35)
        }
        ...
        println("-".repeat(anchos.sum() + 18))

        filas.forEach { fila ->
            val estado = fila[5]
            val estadoColor = if (estado == "DISPONIBLE")
                "${BG_GREEN}${BLACK}${estado.padEnd(anchos[5])}$RESET"
            else
                "${BG_RED}${WHITE}${estado.padEnd(anchos[5])}$RESET"

            val columnas = listOf(
                fila[0].padEnd(anchos[0]),
                fila[1].padEnd(anchos[1]),
                fila[2].padEnd(anchos[2]),
                fila[3].padEnd(anchos[3]),
                fila[4].padEnd(anchos[4]),
                estadoColor,
                fila[6].padEnd(anchos[6])
            )
